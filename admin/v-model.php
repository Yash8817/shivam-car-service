  ...
  <div class="container grid px-6 mx-auto">
    ...
    <div class="w-full overflow-hidden rounded-lg shadow-xs">
      <div class="w-full overflow-x-auto">
        ...
        $offset = ($page - 1) * $limit;
        $sql = "SELECT model.* , maker.maker_name ,car_type.name   from model JOIN maker ON model.Makermaker_id = maker.maker_id JOIN car_type ON maker.car_typecar_type_id = car_type.car_type_id
        LIMIT {$offset},{$limit}";
        ...
        ?>
          <table class="w-full whitespace-no-wrap" id="tbl_body">
            <label class="block text-sm">
              ...
              <input name="model_find" id="model_find" type="text" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" />
            </label><br>
            <thead>
                  ...
                  <td class="px-4 py-3">
                    <div class="flex items-center space-x-4 text-sm">
                      <button class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Edit">
                        <a href="add-v-model.php?id=<?php echo $row['car_model_id']; ?>">
        ...
        <div class="grid px-4 py-3 text-xs font-semibold tracking-wide text-gray-500 uppercase border-t dark:border-gray-700 bg-gray-50 sm:grid-cols-9 dark:text-gray-400 dark:bg-gray-800">
          ...
          <span class="flex col-span-4 mt-2 sm:mt-auto sm:justify-end" id="pagination">
            ...
            if (mysqli_num_rows($result1) > 0) {
              ...
              if ($page > 1) {
                echo '<li><a href="v-model.php?page=' . ($page - 1) . '">Prev&nbsp;</a></li>' . "  ";
              }
              for ($i = 1; $i <= $total_page; $i++) {
                ...
                echo '<li class="' . $active . ' "> &nbsp; <a href="v-model.php?page=' . $i . '">' . $i  . '&nbsp;</a></li>';
              }
              if ($total_page > $page) {
                echo '<li><a href="v-model.php?page=' . ($page + 1) . '">&nbsp;Next</a></li>';
              }
...
<script>
  $(document).on("keyup", "#model_find", function() {
    var model_find = $("#model_find").val();
    $.ajax({
      url: 'getData.php',
      type: 'POST',
      data: {
